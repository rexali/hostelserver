...
export async function searchHostelRoomsBookingHandler(req: Request, res: Response, next: NextFunction) {
    try {
        const terms = req.query as unknown as Terms;
        const hotelRoomsBooking = await HostelService.searchHostelWithTerms(terms,terms.page);
